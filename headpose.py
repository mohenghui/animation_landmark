...
class PoseEstimator:
    ...
    def __init__(self, img_size=(480, 640)):
        self.size = img_size
        # 3D model points.
        self.model_points_3 = np.array([
            (0.0, 0.0, 0.0),             # Nose tip
            (0.0, -330.0, -65.0),        # Chin
            (0.0, 170.0, -135.0),     # eyebrow
            # Right mouth corner
        ], dtype=float)
        self.model_points_4 = np.array([
            (0.0, 0.0, 0.0),             # Nose tip
            (0.0, -330.0, -65.0),        # Chin
    ...
    def solve_pose_by_6_points(self, image_points):
        ...
        """
        points_6 = np.float32([
            image_points[0], image_points[1], image_points[2], image_points[3], image_points[4], image_points[5]])
    ...
    def solve_pose_by_4_points(self, image_points):
        ...
        _, rotation_vector, translation_vector = cv2.solvePnP(
            self.model_points_4,  # 三维坐标 ，float
            points_4,  # 二维坐标
            self.camera_matrix,  # 内参矩阵
            self.dist_coeefs,  # 相机畸变参数
            rvec=self.r_vec,  # 输出旋转向量
    ...
    def solve_pose_by_3_points(self, image_points):
        """
        Solve pose from image points
        Return (rotation_vector, translation_vector) as pose.
        """
        ...
        _, rotation_vector, translation_vector = cv2.solvePnP(
            self.model_points_3,  # 三维坐标 ，float
            points_3,  # 二维坐标
            self.camera_matrix,  # 内参矩阵
            self.dist_coeefs,  # 相机畸变参数
            rvec=self.r_vec,  # 输出旋转向量
...
def run(point_list, img):
    points_3 = point_list  # 加载3个人脸特征点,自行实现

    # img = cv2.imread(pic_path)
    pose_estimator = PoseEstimator(img_size=img.shape)
    # pose = pose_estimator.solve_pose_by_3_points(points_3)
    # pose = pose_estimator.solve_pose_by_4_points(points_3)
    pose = pose_estimator.solve_pose_by_6_points(points_3)
    ...
    for _, txt in enumerate(line.split('\n')):
        cv2.putText(img, txt, (20, y), cv2.FONT_HERSHEY_PLAIN,
                    1.3, (0, 0, 255), 1)
        y = y + 15
    ...
    cv2.line(img, beint(point_list[0]), beint(
             pose[2][0].ravel()), (255, 0, 0), 3)  # BLUE
    cv2.line(img, beint(point_list[0]), beint(
        pose[2][2].ravel()), (0, 0, 255), 3)  # RED
    # for i in pose[2][2].ravel():
    tmp_pose = tuple([int(i)
                      for i in pose[2][2].ravel()])
    tmp_y = tmp_pose[1]-point_list[0][1]
    tmp_x = tmp_pose[0]-point_list[0][0]
    if tmp_x == 0:
        k = inf
    else:
        k = -(tmp_y/tmp_x)
